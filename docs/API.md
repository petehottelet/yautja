...
The shared `target_fill="stroked"` also outlines the analysis disk's outer circle, inner circle and crosshair with a clear interior. `auto` and `filled` retain the translucent disk. Set `hud_colors="analysis-target=#FF4038"` with the Fremont preset for the red stroke shown in the contact sheet; this role controls all stroked contours.
