...
def parser():
    ...
    p.add_argument('--thermal-levels', type=int, help='Representative thermal levels: 2–64, or 0 for continuous. Omit to use mode-specific grading')
    ...
    p.add_argument('--thermal-black-point', type=float, default=0., help='Synthetic warmth mapped to black/cold end, 0–0.95; requires --thermal-levels or --stylepreset')
    ...
    p.add_argument('--target-fill', choices=['auto', 'filled', 'stroked'], default='auto', help='All reticles, including the analysis scan disk: design-specific default treatment, filled artwork, or hollow outlines')
    ...
    p.add_argument('--target-motif-speed', type=float, default=1., help='Triangle rise, contraction and spin speed, 0-5; 0 freezes the ornaments')
    p.add_argument('--target-motif-breaks', type=float, default=0., help='Seeded irregular gaps in triangle edges, 0-1; 0 keeps continuous edges')
    ...
    p.add_argument('--subject-code', action=argparse.BooleanOptionalAction, default=False, help='Show rising glyphs or light streams on detected subjects; requires a segmented mode')
    p.add_argument('--code-style', choices=['glyphs', 'light'], default='glyphs', help='Subject stream artwork: glyphs or continuous light trails centered on the body core; Relic uses light')
    ...
    p.add_argument('--hud-opacity', type=float, default=1., help='Shared HUD visibility, 0-1: 0 is transparent, 1 is full opacity (default); includes outlines and glow')
