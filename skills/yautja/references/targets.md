...
The three lock dots in `triangle-dots` retain their 12% smaller diameter, with center-to-center spacing reduced by 15% around the cluster's existing center. Acquisition timing is unchanged. The `round-dot` dots retain their size and spacing. The README shows these two animations at normal source speed and 24 fps, followed by a [complete target contact sheet](https://github.com/petehottelet/yautja/blob/main/assets/examples/target-shapes.png) of all 11 geometric shapes plus Fremont's scan disk. Geometric shapes appear filled and stroked, with lock dots visible after acquisition. Fremont uses `--analysis-target` and the shared `--target-fill` control. Its filled disk and stroked outer circle, inner circle and crosshair are shown in red on the same neutral background. This color override is confined to the PNG; the preset retains its original translucent gray fill and dark marks. Set `--target-fill stroked --hud-colors "analysis-target=#FF4038"` for the illustrated outlines, or `--target-fill filled --hud-colors "analysis-target-fill=#FF4038"` for the illustrated fill. See [Fremont's analysis controls](analysis.md).
