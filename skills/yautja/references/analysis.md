...
The shared `--target-fill` control also applies to this scan disk. `auto` and `filled` retain its translucent disk and dark targeting marks. `stroked` leaves the interior clear and traces the outer circle, inner circle and crosshair, all using the `analysis-target` color/opacity/blur/neon role. The `analysis-target-fill` layer is inactive in stroked mode. See the [red outlined example](examples.md#fremont-stroked).
