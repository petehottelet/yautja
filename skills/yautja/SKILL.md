...
Focus and Relic use a purple spherical grid with thicker broken edges and a fade toward the center. Pulsing dots and seven-sided node rings are controlled by `--geo-grid-details`; `--geo-grid-speed 0` freezes their motion. Their thick partial bluish-lavender/white edges use `--outline-style holographic --outline-width 8`. Yellow `--target-weak-spots` patches are decorative scan graphics on the selected subject, not physical weakness detection; `--no-target-weak-spots` disables them. The `target-weak-spots` HUD role controls color, opacity, blur and neon. Relic alone enables dense upward code behind subjects and triangle ornaments that rise, contract, spin and disappear. Use `--geo-grid-center-fade`, `--geo-grid-width`, `--geo-grid-breaks`, `--target-motif-speed` and `--target-motif-breaks` to tune these independently; speed 0 freezes ornaments. `--code-density` accepts 0–3 without changing glyph size; Relic uses 1.65. See [Focus and Relic controls](references/focus.md).
...
Choose `--target-shape triangle-dots` for three center dots appearing on lock, `crosshair` for a circular sight, `hollow-cross` for four thick L-shaped bands with a hollow center and open arm ends, `round-dot` for a circular outline with four gaps and three center dots arranged in a triangle, appearing on lock, or `square`, `square-cross`, `square-mil`, and `square-x` for bracketed designs. Hollow Cross replaces Vector Lock; `vector-lock` and `iron-sights` remain aliases for the replacement. All shapes share colors, flash, stroke, blur, opacity, and acquisition settings; `triangle` remains the default. The crosshair, Hollow Cross, round, and square styles lock at 85% of their pre-2.5.7 size; `--target-scale` adjusts their final size. See the target reference for geometry and lock behavior.
...
For a thick, mirrored, nearly full-height waveform, use `--wave-style rorschach` (filled), `rorschach-split` (separated lobes), or `rorschach-hollow` (dark pockets). For chunky digital distortion, choose `digital-blocks` (slabs and square cutouts), `digital-shards` (detached packets), or `digital-circuit` (rounded horizontal bars containing vertical LED segments, with bright active segments that expand with audio and dim inactive segments that stay visible during silence). The default remains `trace`. Set styled waveform width/height with `--wave-width 0.14 --wave-height 1` as fractions of the frame; use `--wave-width 0.09 --wave-height 1` for the narrow, full-height vocoder example. Use `--wave-detail` from 0–1 for smoother Rorschach contours at 0 or sharper audio-driven edge spikes and more intricate lobes toward 1 (default 0.6); in Blocks/Shards it controls pixel density, and in Vocoder Bars it controls row count. The waveform uses its existing audio/procedural source and color; it replaces only the left trace, scale, and flanking glyph rows. Read [waveform details](references/targets.md#waveforms) for ranges and examples.
