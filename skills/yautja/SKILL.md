...
Focus and Relic use a purple spherical grid with thicker broken edges and a fade toward the center. Relic alone enables dense upward code behind subjects and triangle ornaments that rise, contract, spin and disappear. Use `--geo-grid-center-fade`, `--geo-grid-width`, `--geo-grid-breaks`, `--target-motif-speed` and `--target-motif-breaks` to tune these independently; speed 0 freezes ornaments. `--code-density` accepts 0–3 without changing glyph size; Relic uses 1.65. See [Focus and Relic controls](references/focus.md).
